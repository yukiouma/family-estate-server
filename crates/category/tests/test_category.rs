...
async fn category() -> anyhow::Result<()> {
    ...
    uc.add_category(&Category { id: None, name: "main_demo".into() }).await?;
    let mut categories = uc.list_categories().await?;
    assert_eq!(categories.len(), 1);
    let category = categories.first_mut().unwrap();
    uc.add_sub_category(category.id.unwrap(), &Category { id: None, name: "sub_demo".into() }).await?;
    ...
    assert_eq!(uc.list_categories().await?.first().unwrap().name, "main_update");
    sub_category.name = "sub_update".into();
    uc.modify_category(&sub_category).await?;
    assert_eq!(uc.list_sub_categories(category.id.unwrap()).await?.first().unwrap().name, "sub_update");
